...
double CalcNSMaxDeg(int *deg, int &max_deg, double eps, string outfile)
{
	...
	for (i = 0; i < NodeNum; i++)
	{
		...
		if (max_deg < deg[i])
			max_deg = deg[i];
		if (max_deg_ns < deg_lap[i])
			max_deg_ns = deg_lap[i];
	}

	// If max_deg_ns exceeds NodeNum - 1, then use NodeNum - 1
	if (max_deg_ns > (double)NodeNum - 1.0)
		max_deg_ns = (double)NodeNum - 1.0;
...
void CalcNLocTri(map<int, int> *a_mat, int *deg, string outfile, double &tri_num_ns, int emp)
{
	...
	tot_edge_num_ns = 0;
	for (i = 0; i < NodeNum; i++)
		tot_edge_num_ns += (long long)deg_ns[i];
	tot_edge_num_ns /= 2;
			...
			for (i = 0; i < NodeNum; i++)
			{
				...
				if (deg[i] > max_deg_ns)
				{
					...
					for (aitr = a_mat[i].begin(); aitr != a_mat[i].end(); aitr++)
					{
						...
						if (a_mat_del[i].count(j) == 1)
							continue;
						for (aitr2 = a_mat[i].begin(); aitr2 != a_mat[i].end(); aitr2++)
						{
							k = aitr2->first;
							// Continue if the edge is deleted
							if (a_mat_del[i].count(k) == 1)
								continue;
							if (j >= k || k >= i)
								continue;
							st2_num_u[i] += 1.0;
...
double CalcClstCoef(double tri_num_ns, double st2_num_ns)
{
	...
	if (st2_num_ns == 0)
		...
		clst_ns = 3.0 * tri_num_ns / st2_num_ns;
	if (clst_ns > 1.0)
		clst_ns = 1.0;
	else if (clst_ns < 0.0)
		clst_ns = 0.0;
...
int main(int argc, char *argv[])
{
	...
	for (itr = 0; itr < ItrNum; itr++)
	{
		...
		if (st2_num != 0)
			clst = 3.0 * (double)tri_num / (double)st2_num;
		else
			clst = 1.0;
